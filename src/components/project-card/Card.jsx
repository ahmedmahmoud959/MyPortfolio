  ...
  return (
    <Fragment>
      <button onClick={() => setShowDetails(true)}>
        <div className="pro-card max-h-72 group w-full overflow-hidden relative hover:cursor-pointer border-[3px] rounded-xl hover:border-primary border-primary-op transition-all duration-700 ease-in-out">
          {data.images[0] && (
            <img
              src={data.images[0]}
              alt="project"
      ...
      <Modal status={showDetails} data={data} onHide={() => setShowDetails(false)} />
    </Fragment>
  );
}
